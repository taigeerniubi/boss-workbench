...
$env:BOSS_CHROME_MODE = "normal"                     # 显示窗口（默认隐藏，见下）
```
...
插件默认用 `--headless=new` **隐藏模式**拉起浏览器：没有窗口，但调试口、登录态、抓取、
读会话都照常工作。这条路已在本机实测（Edge 153 + 插件 profile）：调试口即刻就绪，
wt2 / `__zp_stoken__` 都在，`navigator.webdriver=false`。早先 README 里说"没能验证"，
是因为当时在受限 shell 里连正常 Chrome 都起不来，实测被污染了。

隐藏模式顺手抹掉两个无头特征：UA 里的 `HeadlessChrome/` 换成同版本正常写法（版本号取
exe 旁的版本目录，退到 profile 的 `Last Version`），窗口尺寸给成 1440×900（默认 800×600）。

为什么改成默认隐藏：可见窗口的实际结果是用户顺手把它关掉，下次抓取又拉一个 ——
"开一下又关掉"的循环。隐藏实例不会被人误关，会一直挂在后台直到 DSH 退出。

**扫码登录需要看得见的窗口。** 点「登录」时插件发现没登录态，会先让隐藏实例正常退出
（CDP `Browser.close`，不是杀进程），再以可见模式重拉同一个 profile —— 同一个 profile
不能并存两个实例，直接再起一个只会被合并进无头的那个。登录完那个窗口留着不动；
关掉后下次抓取重新拉一个隐藏实例，登录态在 profile 里，不用再扫。

想一直看见窗口：`BOSS_CHROME_MODE=normal`。此时**关掉那个窗口＝断掉插件的通道**，
抓取和读会话会失败，直到重新打开。
