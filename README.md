...
- **撞过风控之后工具会自己锁上**：`code 35` 锁 2 小时、`code 37` 锁 20 分钟。
  期间所有抓取入口直接拒绝执行（工作台上的按钮变红并写明还剩多久），
  状态在 `~/.dsh/boss-workbench/data/cooldown.json`。
  这是**故意的** —— "再试一次"是排障时最本能的动作，也是最该被拦下来的那个。
  真要强行跑：`node boss/scrape.mjs … --ignore-cooldown`。
